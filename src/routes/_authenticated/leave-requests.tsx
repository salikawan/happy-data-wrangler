  ...
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("leave_requests")
        .select("*, profiles:profiles!leave_requests_user_id_fkey(full_name, email, employee_id)")
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
          ...
          return (
            <div key={r.id} className="rounded-2xl border bg-card p-4 shadow-sm flex flex-wrap items-center gap-3">
              <div className="flex-1 min-w-0">
                <div className="font-medium">{p?.full_name || p?.email} <span className="text-xs text-muted-foreground">{p?.employee_id}</span></div>
                <div className="text-xs text-muted-foreground">{r.leave_type} • {r.from_date} → {r.to_date}</div>
        ...
        {rows.length === 0 && <div className="rounded-2xl border bg-card p-8 text-center text-sm text-muted-foreground">No leave requests yet.</div>}
      </div>
    </div>
  );
}
